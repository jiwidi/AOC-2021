...
def convert_to_absolute(scanner_location, points):
    ...
    for point in points:
        new.append(
            Point(
                point.x + scanner_location.x,
                point.y + scanner_location.y,
                point.z + scanner_location.z,
            )
        )
...
def locate_scanners(scanners):
    n = len(scanners)
    located_scanners = {0: Scanner(scanners[0].detected_points, Point(0, 0, 0))}
...
def solve_part_2(scanners):
    ...
    for i in range(len(located_scanners)):
        for j in range(i + 1, len(located_scanners)):
            ...
            res = max(res, scanner_1.location.manhattan_dist(scanner_2.location))
